...
{
    /** ディレクトリの作成 **/
    const char dir_0[] = "Estimated_position_GPS";
    const char dir_1[] = "Estimated_position_GPS/position";
    const char dir_2[] = "Estimated_position_GPS/route";
    const char dir_3[] = "Estimated_position_GPS/graph";
...
void Write_data(int n)
{
    ...
    /** 走行位置の書き出し **/
    char filename[100];
    sprintf(filename, "Estimated_position_GPS/position/%d.dat", n);
    fp = fopen(filename, "w");
    fprintf(fp, "%f\t%f\t%f\n", t, x[n], y[n]);
    fclose(fp);

    /** 走行経路の書き出し **/
    sprintf(filename, "Estimated_position_GPS/route/%d.dat", n);
    fp = fopen(filename, "w");
    for (int i = 0; i <= n; i++)
    {
...
void Gnuplot(int n)
{
    ...
    sprintf(filename_1, "Estimated_position_GPS/position/%d.dat", n);
    sprintf(filename_2, "Estimated_position_GPS/route/%d.dat", n);
    sprintf(graphname, "Estimated_position_GPS/graph/%04d.png", n);
