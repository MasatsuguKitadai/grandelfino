...
{
    /** ディレクトリの作成 **/
    const char dir_0[] = "SLAM+GPS";
    const char dir_1[] = "SLAM+GPS/position";
    const char dir_2[] = "SLAM+GPS/route";
    const char dir_3[] = "SLAM+GPS/graph";
...
void Write_data(int n)
{
    ...
    /** 走行位置の書き出し **/
    char filename[100];
    sprintf(filename, "SLAM+GPS/position/%d.dat", n);
    fp = fopen(filename, "w");
    fprintf(fp, "%f\t%f\t%f\n", t, x[n], y[n]);
    fclose(fp);

    /** 走行経路の書き出し **/
    sprintf(filename, "SLAM+GPS/route/%d.dat", n);
    fp = fopen(filename, "w");
    for (int i = 0; i <= n; i++)
    {
...
void Gnuplot(int n)
{
    ...
    sprintf(filename_1, "SLAM+GPS/position/%d.dat", n);
    sprintf(filename_2, "SLAM+GPS/route/%d.dat", n);
    sprintf(graphname, "SLAM+GPS/graph/%04d.png", n);
